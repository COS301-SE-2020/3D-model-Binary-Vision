...
bool SfmPipeline::createImageListings()
{
	...
	*/
	...
	// Create Directory with the same name as the passed file_name (LINUX)
	string dir_path = getCurrentDir() + "imageMatches/" + file_name;
	if (!Helper::directoryExist(dir_path)){
		if(!Helper::createDirectory(dir_path)){
			return false;
		}
	}
...
bool SfmPipeline::computeIncrementalSfm()
{
	...
	*/
		...
		// Create Directory where the final reconstruction will be stored. (LINUX)
		string dir_path = getCurrentDir() + "sfmReconstruction/" + file_name;
		if (!Helper::directoryExist(dir_path)){
			if(!Helper::createDirectory(dir_path)){
				return false;
...
void SfmPipeline::applyConfiguration(std::ifstream* _file)
{
    ...
    std::getline(*_file, line);
    setFocalLength(Helper::readProperty(line));
    std::getline(*_file, line);
    setCameraModel(Helper::readProperty(line));
    std::getline(*_file, line);
    std::getline(*_file, line);
    std::getline(*_file, line);
    setForceFeature(Helper::readProperty(line));
    std::getline(*_file, line);
    setDescriberMethod(Helper::readProperty(line));
    std::getline(*_file, line);
    setUpright(Helper::readProperty(line));
    std::getline(*_file, line);
    setDescriberQuality(Helper::readProperty(line));
    std::getline(*_file, line);
    std::getline(*_file, line);
    std::getline(*_file, line);
    setForceMatchs(Helper::readProperty(line));
    std::getline(*_file, line);
		setRatio(Helper::readProperty(line));
    std::getline(*_file, line);
    setGeoModel(Helper::readProperty(line));
    std::getline(*_file, line);
    setNearestMatching(Helper::readProperty(line));
    std::getline(*_file, line);
    std::getline(*_file, line);
    std::getline(*_file, line);
    mode = line;
    setSfmMode(Helper::readProperty(line));
    std::getline(*_file, line);
    std::getline(*_file, line);
    std::getline(*_file, line);
    if (mode == "Incrimental") {
        setRefineIntrinsic(Helper::readProperty(line));
    }
    else {
        std::getline(*_file, line);
        std::getline(*_file, line);
        std::getline(*_file, line);
        setRotationAveraging(Helper::readProperty(line));
        std::getline(*_file, line);
        setTranslationAveraging(Helper::readProperty(line));
        std::getline(*_file, line);
        setRefineIntrinsic(Helper::readProperty(line));
    }
    _file->close();
